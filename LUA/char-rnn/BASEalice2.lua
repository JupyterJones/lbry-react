...
for i = 1, iterations do
    ...
    if i % opt.eval_val_every == 0 or i == iterations then
        ...
        savedata = string.format('%s/%s, %s, %.12f, %s, %6.8f, %.2f, %.4f.t7', opt.checkpoint_dir, opt.savefile, ScriptName, optim_state.learningRate, opt.rnn_size, train_loss, epoch, val_loss)
        db:exec('INSERT INTO test VALUES ("'..savedata..'")')
    ...
    if i % opt.print_every == 0 then
        ...
        info = string.format("%s, %d, %d, %.3f, %6.8f, %6.4e, %.4fs", ScriptName, i, iterations, epoch, train_loss, grad_params:norm() / params:norm(), time)
