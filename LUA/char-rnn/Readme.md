# My Changes
  uranbible/input.txt is a combination of several files. The Bible, Urantia, Enoch and more.
  The Python files will use Lua to create plots, create text files from trained models, and print text in terminal.
  Also the history of each iteration printed in terminal and saved in files is kept in a sqlite database. The 'save'
  filenames have modified to contain data that can be parsed.
  When any of the python files are run they generate a data file ( name based on the directory ) It is generated by listing   the desired directory by date and parsing the loss from the filename then entering the list in a the file. Example:
  unranbible_.dat  this data file contains the loss of each iteration. example:
  3.1717, 2.4099, 2.0061, 1.8144, 1.6780, 1.5947, 1.5294, 1.4846, 1.4561, etc
  mkplot.py  creates a plot, generated by data in the filename  
  mktext-file.py  creates a text file asks filename and seeding text.
  mktext-n-plot.py creates text in terminal then makes a plot  
  mktext.py creats text only
  THE USAGE: Each of the files require a directory option
  EXAMPLE - python mkplot.py uranbible
  EXAMPLE - python mktext-file uranbible 
  .... etc 
...
(Ubuntu - you must have libsqlite3-dev installed first then install lsqlite3)
